...
private:
  ...
  bool matchR8AbsGlobal(SDValue Addr, SDValue &SymOut) const;
...
// Match (S1C33ISD::Wrapper (TargetGlobalAddress | TargetExternalSymbol |
// TargetBlockAddress | TargetConstantPool)).  On success, returns the
// wrapped symbol SDValue.
//
// We do NOT gate on hasOneUse or per-function reference counts: although for
// multi-access cases the materialize-once + reuse-register path spends fewer
// text bytes, the ext/ext/[%r8] pattern compresses very well in pex (P/ECE's
// LZ-based packer).  Empirically the pex size goes down overall when we
// always emit the fused form, mirroring gcc33's behavior.
bool S1C33DAGToDAGISel::matchR8AbsGlobal(SDValue Addr, SDValue &SymOut) const {
  if (Addr.getOpcode() != S1C33ISD::Wrapper)
    return false;
  SDValue Inner = Addr.getOperand(0);
  unsigned Op = Inner.getOpcode();
  if (Op != ISD::TargetGlobalAddress && Op != ISD::TargetExternalSymbol &&
      Op != ISD::TargetBlockAddress && Op != ISD::TargetConstantPool)
    return false;
  SymOut = Inner;
  return true;
}
...
void S1C33DAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::LOAD: {
    ...
    // R8 absolute addressing: (load (S1C33Wrapper sym)) where the wrapper has
    // exactly one use.  Emits `ext sym@ah / ext sym@al / ld.* [%r8]` in 6
    // bytes, saving 2 bytes vs the materialize+indirect-load path (8 bytes).
    if (Subtarget->hasR8AbsGlobal()) {
      SDValue AbsSym;
      if (matchR8AbsGlobal(Addr, AbsSym)) {
        unsigned AbsOpc;
        if (MemVT == MVT::i32)
          AbsOpc = S1C33::LDW_ABS;
        else if (MemVT == MVT::i8 && Ext == ISD::ZEXTLOAD)
          AbsOpc = S1C33::LDUB_ABS;
        else if (MemVT == MVT::i8)
          AbsOpc = S1C33::LDB_ABS;
        else if (MemVT == MVT::i16 && Ext == ISD::ZEXTLOAD)
          AbsOpc = S1C33::LDUH_ABS;
        else if (MemVT == MVT::i16)
          AbsOpc = S1C33::LDH_ABS;
        else
          AbsOpc = 0;
        if (AbsOpc) {
          SDNode *New = CurDAG->getMachineNode(
              AbsOpc, DL, {MVT::i32, MVT::Other}, {AbsSym, LD->getChain()});
  ...
  case ISD::STORE: {
    ...
    // R8 absolute addressing for single-use global stores — see LOAD above.
    if (Subtarget->hasR8AbsGlobal()) {
      SDValue AbsSym;
      if (matchR8AbsGlobal(Addr, AbsSym)) {
        unsigned AbsOpc;
        if (MemVT == MVT::i32)
          AbsOpc = S1C33::STW_ABS;
        else if (MemVT == MVT::i8)
          AbsOpc = S1C33::STB_ABS;
        else if (MemVT == MVT::i16)
          AbsOpc = S1C33::STH_ABS;
        else
          AbsOpc = 0;
        if (AbsOpc) {
          SDNode *New = CurDAG->getMachineNode(
