...
S1C33TargetLowering::S1C33TargetLowering(const TargetMachine &TM,
  ...
  // Register DAG combines for sign-bit test optimization:
  // sext_inreg(x, i8/i16) <s 0  →  (x & signbit) != 0
  // Avoids 3×sll + 3×sra + cmp sequence generated by InstCombine.
  setTargetDAGCombine({ISD::BR_CC, ISD::SETCC});
}
